...
navOld.addEventListener('click', () => {
  ...
  const oldList = temples.filter(temple => {
...
navNew.addEventListener('click', () => {
  ...
  const newList = temples.filter(temple => {
    const year = parseInt(temple.dedicated.split(',')[0], 10);
    return year > 2000;
  });
...
navLarge.addEventListener('click', () => {
  ...
  let largeList = temples.filter(temples => temples.area > 90000);
  createTempleCard(largeList);
});

navSmall.addEventListener('click', () => {
  header.textContent = 'Small Temples';
  let smallList = temples.filter(temples => temples.area < 10000);
